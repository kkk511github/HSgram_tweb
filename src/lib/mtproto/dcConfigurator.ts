...
function formatCustomUrl(
  ...
  return url
  .split('{dcId}').join('' + dcId)
  .split('{connectionType}').join(connectionType)
...
export function constructTelegramWebSocketUrl(dcId: DcId, connectionType: ConnectionType, premium?: boolean) {
  ...
  if(customServer) {
    return formatCustomUrl(customServer, dcId, connectionType, suffix, path);
  }
...
export class DcConfigurator {
  ...
  private transportHTTP = (dcId: DcId, connectionType: ConnectionType, premium?: boolean) => {
    ...
    const customServer = import.meta.env.VITE_MTPROTO_HTTP_URL;
    if(customServer) {
      const suffix = getTelegramConnectionSuffix(connectionType);
      const path = Modes.test ? 'apiw_test1' : 'apiw1';
      chosenServer = formatCustomUrl(customServer, dcId, connectionType, suffix, path);
